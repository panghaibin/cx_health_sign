...
3. 在新建 Secret 的界面上， `Name` 值输入 `USERS` ， `Value` 处输入用户的配置信息，多个用户配置之间使用`英文分号`隔开。
   ...
   用户需要消息推送服务，并且使用学号登录：
   ```
   20192233,12345Abc,default,18,2,5e58d2264821c69ebcd46c448e7f5fe6
   ```
   
   若用户不使用学号登录，或者不需要使用消息推送服务，则**按照上述格式的顺序**，仅保留需要的参数即可
   
   例如用户不需要消息推送服务，使用手机号而不是学号登录：
   
   ```
   13878000000,12345Abc,default
   ```
   ...
   ```
   20192233,12345Abc,default,209,2,5e58d2264821c69ebcd46c448e7f5fe6;13878000000,12345Abc,default
   ```
